...
const Marketplace = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      let query = supabase.from("products")
        .select("id,title,price,currency,era,location,images,category_slug,profiles(username,verified)")
        .eq("status", "active");
  ...
  const heading = useMemo(() => {
    ...
    return categories.find((c) => c.slug === category)?.name || t("marketplace_label");
