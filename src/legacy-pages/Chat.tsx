...
import { categoryLabel, useLanguage, LANGS } from "@/lib/language";
...
const salonCopy: Record<string, Record<string, string>> = {
  global: { en: "Global Salon", fr: "Salon mondial", ar: "الصالون العالمي", tr: "Global Salon" },
  rooms: { en: "Rooms", fr: "Salles", ar: "الغرف", tr: "Odalar" },
  title: { en: "The Salon", fr: "Le Salon", ar: "الصالون", tr: "Salon" },
  subtitle: { en: "Where collectors gather. Pick a room, share finds, translate, and call.", fr: "Le rendez-vous des collectionneurs. Choisissez une salle, partagez, traduisez et appelez.", ar: "حيث يجتمع هواة جمع التحف. اختر غرفة وشارك وترجم واتصل.", tr: "Koleksiyonerlerin buluşma noktası. Oda seçin, paylaşın, çevirin ve arayın." },
  greeting: { en: "Be the first to greet the salon.", fr: "Soyez le premier à saluer le salon.", ar: "كن أول من يحيّي الصالون.", tr: "Salonu ilk selamlayan siz olun." },
  placeholder: { en: "Speak to the salon…", fr: "Parlez au salon…", ar: "تحدث إلى الصالون…", tr: "Salona yazın…" },
  signIn: { en: "Sign in to send messages and join calls.", fr: "Connectez-vous pour envoyer des messages et rejoindre les appels.", ar: "سجّل الدخول لإرسال الرسائل والانضمام للمكالمات.", tr: "Mesaj göndermek ve aramalara katılmak için giriş yapın." },
  join: { en: "Join the Club", fr: "Rejoindre le Club", ar: "انضم للنادي", tr: "Kulübe Katıl" },
  leave: { en: "Leave call", fr: "Quitter l’appel", ar: "مغادرة المكالمة", tr: "Aramadan ayrıl" },
  video: { en: "Video Call", fr: "Appel vidéo", ar: "مكالمة فيديو", tr: "Görüntülü Arama" },
  voice: { en: "Voice Call", fr: "Appel vocal", ar: "مكالمة صوتية", tr: "Sesli Arama" },
};
const roomLabel = (room: Room, lang: string) => categoryLabel(room.slug, lang, room.name);
const copy = (key: string, lang: string) => salonCopy[key]?.[lang] || salonCopy[key]?.en || key;
...
const Chat = () => {
  ...
  const startCall = (type: "video" | "audio") => {
    ...
    // Use a deterministic Jitsi room so salon calls work without a missing
    // Daily.co server key. The room is shared by everyone in this salon.
    const room = `antiqueclub-salon-${activeRoom.slug}`;
    setCallType(type);
    // Jitsi reads its interface language from the URL config. Keep the call
    // aligned with the language selected in the Salon language picker.
    const callLanguage = targetLang === "zh" ? "zhCN" : targetLang === "pt" ? "ptBR" : targetLang;
    const mode = type === "audio"
  ...
  if (callUrl && callIframeUrl) {
    return (
      <main className="fixed inset-0 z-50 bg-ink">
        <div className="absolute left-0 right-0 top-0 z-10 flex items-center justify-between border-b border-primary-foreground/20 bg-ink/90 px-4 py-3 text-primary-foreground backdrop-blur">
          <div className="flex items-center gap-2 font-display text-lg">
            {callType === "video" ? <Video className="h-5 w-5 text-gold" /> : <Phone className="h-5 w-5 text-gold" />}
            {callType === "video" ? copy("video", targetLang) : copy("voice", targetLang)} · {activeRoom ? roomLabel(activeRoom, targetLang) : ""}
          </div>
          <Button variant="ghost" size="sm" onClick={() => setCallUrl(null)} className="text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground">
            <PhoneOff className="mr-1 h-4 w-4" /> {copy("leave", targetLang)}
          </Button>
        </div>
        <iframe
          src={callIframeUrl}
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="container py-6 flex-1 flex flex-col max-w-7xl">
        <div className="mb-4">
          <div className="font-serif-cap text-xs text-gold-dark mb-1 flex items-center gap-2"><Globe className="h-3.5 w-3.5" /> {copy("global", targetLang)}</div>
          <h1 className="font-display text-4xl">{copy("title", targetLang)}</h1>
          <p className="text-muted-foreground text-sm">{copy("subtitle", targetLang)}</p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-[240px,1fr] flex-1 min-h-[70vh]">
          ...
          <aside className="frame-card p-3 h-fit md:sticky md:top-24">
            <div className="font-serif-cap text-[11px] text-gold-dark px-2 pb-2">{copy("rooms", targetLang)}</div>
              ...
              {rooms.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-medium truncate">{roomLabel(r, targetLang)}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="frame-card flex flex-col overflow-hidden min-h-[70vh]">
            {activeRoom && (
              <div className="border-b border-border/60 px-5 py-3 flex items-center gap-3 bg-gradient-parchment">
                ...
                <Button variant="outline" size="sm" disabled={!activeRoom} onClick={() => startCall("audio")} title="Voice call">
                  <Phone className="h-4 w-4" />
                </Button>
                <Button size="sm" disabled={!activeRoom} onClick={() => startCall("video")} className="bg-gradient-gold text-ink shadow-gold" title="Video call">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-gradient-parchment">
              {msgs.length === 0 && <p className="text-center text-muted-foreground py-12">{copy("greeting", targetLang)}</p>}
              {msgs.map((m) => {
            ...
            <div className="border-t border-border/60 p-3 flex items-center gap-2 bg-background">
              {user ? (
                <>
                  ...
                  <Input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && (e.preventDefault(), send())} placeholder={copy("placeholder", targetLang)} maxLength={1000} />
                ...
                <div className="flex-1 flex items-center justify-between gap-3 px-2">
                  <span className="text-sm text-muted-foreground">{copy("signIn", targetLang)}</span>
                  <Link to="/auth"><Button size="sm" className="bg-gradient-gold text-ink shadow-gold">{copy("join", targetLang)}</Button></Link>
                </div>
              )}
            </div>
          </div>
